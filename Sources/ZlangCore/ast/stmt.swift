...
class BlockStmt: Stmt {
    ...
    override func str() -> String {
        var str = stmts.map { stmt in "    " + stmt.str() }.joined(
            separator: "\n"
        )
        if stmts.count != 0 {
            str = "\n\(str)\n"
    ...
    override func text() -> String {
        var str = stmts.map { stmt in "    " + stmt.text() }.joined(
            separator: "\n"
        )
        if stmts.count != 0 {
            str = "\n\(str)\n"
    ...
    override func gen() -> String {
        var str = stmts.map { stmt in "    " + stmt.gen() }.joined(
            separator: "\n"
        )
        if stmts.count != 0 {
            str = "\n\(str)\n"
