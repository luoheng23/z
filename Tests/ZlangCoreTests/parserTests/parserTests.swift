...
class BaseTests: XCTestCase {
    ...
    func single<T>(
        ...
        let answer =
            breaker.count == 1
            ? str.split(separator: Character(breaker)) : [str[...]]
        for word in answer {
            let token =
                defaultKind == .expr
                ? parser.expr()
                : (defaultKind == .stmt ? parser.stmt() : parser.decl())
            XCTAssert(
                (type(of: token) == typeClass) == not,
                "Failed to test \(token.str()) with \(typeClass)"
            )
            XCTAssert(
                word == token.text(),
                "Failed to test \(word) != \(token.text())"
            )
        }
    }
